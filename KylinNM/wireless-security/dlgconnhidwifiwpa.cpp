...
DlgConnHidWifiWpa::DlgConnHidWifiWpa(int type, KylinNM *mainWindow, QWidget *parent) :
    isUsed(type),
    QDialog(parent),
    ui(new Ui::DlgConnHidWifiWpa)
{
    ...
    QString sty = "QWidget{border-radius:6px;background-color:rgba(255,255,255,1);border:1px solid rgba(255, 255, 255, 0.05);}";
...
** 手动绑定下拉框视图和下拉框
**/
bool DlgConnHidWifiWpa::eventFilter(QObject *obj, QEvent *ev)
{
...
void DlgConnHidWifiWpa::on_btnConnect_clicked()
{
    ...
    if (isUsed == 0){
        ...
        do{
            ...
            QString cmd = "nmcli device wifi connect \"" + wifiName.replace("\"","\\\"") + "\" password \"" + wifiPassword.replace("\"","\\\"") + "\" hidden yes > " + tmpPath;
...
void DlgConnHidWifiWpa::on_execSecConn()
{
    ...
    QString pwd = strWifiPassword;
    QString str = "nmcli device wifi connect \"" + name.replace("\"","\\\"") + "\" password\"" + pwd.replace("\"","\\\"") + "\"";
