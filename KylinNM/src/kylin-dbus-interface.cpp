    ...
    foreach (QDBusObjectPath localPath, multiWiredPaths) {
        ...
        try {
            if (reply.value().toString() == "true") {
                //isWiredCableOn = true;
                QDBusReply<QVariant> managed = interface.call("Get", "org.freedesktop.NetworkManager.Device", "Managed");
...
void KylinDBus::initConnectionInfo()
{
    ...
    setBtnWifiGsetting(oldWifiSwitchState);
}
...
{
//    qDebug()<<"WIFI连接状态："<<is_wifi_connected;
    if (!m_nm_gsettings) {
        return ;
    }

    if (m_nm_gsettings->keys().contains("wificonn")) {
...
{
//    qDebug()<<"LAN连接状态："<<is_lan_connected;
    if (!m_nm_gsettings) {
        return ;
    }

    if (m_nm_gsettings->keys().contains("lanconn")) {
